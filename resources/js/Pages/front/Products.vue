<template >
    <div class="div">
        <n-layout has-sider position="absolute">
            <n-layout-sider position="absolute" bordered collapse-mode="width" :collapsed-width="10" :width="240"
                :collapsed="collapsed" :show-collapsed-content=false show-trigger="bar" @collapse="collapsed = true"
                ...
                <div class="container" v-if="category_id == null">
                    <div class="container" v-for="category in categories.data" :key="category.id">
                        <n-tree-select class="mt-1" :show-path="true" placeholder='kategoriya saylang'
                            :default-value="category.id" :options="[category]" children-field="children" key-field="id"
                            label-field="name" @update:value="handleUpdateValue" />
                    </div>
                </div>

                <div class="container" v-else>
                    <n-tree-select class="mt-1" :show-path="true" placeholder='kategoriya saylang'
                        :default-value="category_id ? parseInt(category_id) : null" :options="categories.data"
                        children-field="children" key-field="id" label-field="name" @update:value="handleUpdateValue" />
                </div>
                            ...
                            <div :id="'panelsStayOpen-collapse-o' + option.id" class="accordion-collapse"
                                ...
                                <div class="accordion-body px-2 py-1">
                                    <div class="form-check my-2" v-for="value in option.values" :key="value.id">
                                        <input class="form-check-input" type="checkbox" v-model="form.values"
                                            :value='value.id'>
            ...
            <n-layout>
                <Nav />
                <n-input-group class=" justify-content-center">
                    <n-button round  size="large" type="primary">
                        Gozle
                    </n-button>
                    <n-input v-model:value="form.search" size="large" round :style="{ width: '70%' }" />
                </n-input-group>
                <div
...
<script setup>
import { NSpace, NLayout, NSwitch, NLayoutSider, NMenu, NTreeSelect, NInputGroup, NInput, NButton, NBreadcrumb, NBreadcrumbItem, NRow, NCol, NStatistic } from 'naive-ui'
...
function handleUpdateValue(id) {
    //'i am only when category changed;
...
watch([form], ([newValues], [prevValues],) => {
    ...
    Inertia.get('/outfits/home', { c: categoryIdForFilter, v: form.values, search: form.search }, { preserveState: true, replace: true });
})
</script>
<script>
